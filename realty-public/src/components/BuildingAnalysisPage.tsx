...
const ZONE_STORAGE_KEY = 'building_zone_limits';

const DEFAULT_ZONE_LIMITS: Record<string, { bc: number; vl: number }> = {
...
const ZONE_ORDER = Object.keys(DEFAULT_ZONE_LIMITS);

function loadZoneLimits(): Record<string, { bc: number; vl: number }> {
  try {
    const saved = localStorage.getItem(ZONE_STORAGE_KEY);
    if (saved) return { ...DEFAULT_ZONE_LIMITS, ...JSON.parse(saved) };
  } catch {}
  return { ...DEFAULT_ZONE_LIMITS };
}
...
interface GeoCandidate {
  address_name: string;
  x: string; y: string;
  sigunguCd: string; bjdongCd: string;
  bun: string; ji: string; platGbCd: string;
}
...
interface BuildingForm {
  bldNm: string; platArea: string; archArea: string;
  bcRat: string; totArea: string; vlRat: string;
  vlEstmArea: string; grndFlrCnt: string; ugrndFlrCnt: string;
  elevator: string; parking: string; useAprDay: string;
  zoneType: string; districtPlan: string; violation: string;
}

interface UserInput {
  salePrice: string; deposit: string; monthlyRent: string;
  maxBcRat: string; maxVlRat: string;
}
...
function BuildingVisual({
  ...
  grndFlrCnt: number; ugrndFlrCnt: number;
  additionalFloors: number; floorItems: FloorItem[];
}) {
  ...
  type FloorEntry = { label: string; color: string; edge: string; floorKey?: number };
  const floors: FloorEntry[] = [];

  for (let i = 0; i < ugrndFlrCnt; i++) {
    const d = ugrndFlrCnt - i;
    floors.push({ label: `B${d}`, color: '#5c4a3a', edge: '#3d2e20', floorKey: -d });
  }
...
function SettingsTab({
  ...
  zoneLimits: Record<string, { bc: number; vl: number }>;
  onSave: (limits: Record<string, { bc: number; vl: number }>) => void;
}) {
  const [rows, setRows] = useState(() =>
    ZONE_ORDER.map(zone => ({
      zone,
      bc: String(zoneLimits[zone]?.bc ?? DEFAULT_ZONE_LIMITS[zone].bc),
      vl: String(zoneLimits[zone]?.vl ?? DEFAULT_ZONE_LIMITS[zone].vl),
    }))
  );

  useEffect(() => {
    setRows(ZONE_ORDER.map(zone => ({
      zone,
      bc: String(zoneLimits[zone]?.bc ?? DEFAULT_ZONE_LIMITS[zone].bc),
      vl: String(zoneLimits[zone]?.vl ?? DEFAULT_ZONE_LIMITS[zone].vl),
    })));
  ...
  const handleChange = (idx: number, field: 'bc' | 'vl', val: string) => {
    setRows(prev => {
      const next = [...prev];
  ...
  const handleSave = () => {
    const result: Record<string, { bc: number; vl: number }> = {};
    for (const row of rows) {
      const bc = parseFloat(row.bc);
      const vl = parseFloat(row.vl);
      if (isNaN(bc) || isNaN(vl)) {
        alert(`'${row.zone}'의 값이 올바르지 않습니다. 숫자를 입력해주세요.`);
        return;
      }
      result[row.zone] = { bc, vl };
    }
    localStorage.setItem(ZONE_STORAGE_KEY, JSON.stringify(result));
    onSave(result);
    alert('저장되었습니다.');
  };

  const handleReset = () => {
    if (!window.confirm('서울시 도시계획조례 기본값으로 초기화하시겠습니까?')) return;
    setRows(ZONE_ORDER.map(zone => ({
      zone,
      bc: String(DEFAULT_ZONE_LIMITS[zone].bc),
      vl: String(DEFAULT_ZONE_LIMITS[zone].vl),
    })));
  };

  return (
    <div className="bld-settings">
      <div className="bld-settings-header">용도지역별 최대 건폐율 / 용적률</div>
      <p className="bld-settings-desc">
        서울시 도시계획조례 기준 기본값입니다. 수정 후 저장 버튼을 누르세요.
      </p>
      <table className="bld-settings-table">
        <thead>
          <tr>
            <th>용도지역</th>
            <th>최대건폐율 (%)</th>
        ...
        <tbody>
          {rows.map((row, idx) => (
            <tr key={row.zone}>
              ...
              <td>
                <input
                  className="bld-settings-input"
                  value={row.bc}
                  onChange={e => handleChange(idx, 'bc', e.target.value)}
                />
              </td>
              <td>
                <input
                  className="bld-settings-input"
                  value={row.vl}
                  onChange={e => handleChange(idx, 'vl', e.target.value)}
                />
              </td>
            </tr>
          ))}
      ...
      <div className="bld-settings-buttons">
        <button className="bld-btn-reset" onClick={handleReset}>초기화</button>
        <button className="bld-btn-save" onClick={handleSave}>저장</button>
      </div>
    </div>
  );
}
...
export default function BuildingAnalysisPage() {
  ...
  const [zoneLimits, setZoneLimits]   = useState<Record<string, { bc: number; vl: number }>>(loadZoneLimits);
  ...
  const maxFloorsByVl = useMemo(() => {
    ...
    if (vlRatEstm && vlRatEstm > 0) {
      return curr * (maxVl / vlRatEstm);
    }
    // fallback: 건축면적 기준 (파이썬과 동일)
    const platN = toNum(building.platArea);
    const archN = toNum(building.archArea);
    if (platN > 0 && archN > 0) {
      return ((maxVl / 100) * platN) / archN;
    }
    return null;
  }, [building.grndFlrCnt, building.platArea, building.archArea, userIn.maxVlRat, vlRatEstm]);
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      ...
      setCandidates(cands);
      if (cands.length === 1) await selectCandidate(cands[0]);
    } catch (e) {
  ...
  const selectCandidate = useCallback(async (cand: GeoCandidate) => {
    ...
    try {
      const res  = await fetch(`${API_BASE}/api/building/analyze`, {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          sigunguCd: cand.sigunguCd, bjdongCd: cand.bjdongCd,
          platGbCd:  cand.platGbCd,  bun:      cand.bun,
          ji:        cand.ji,         x:        cand.x, y: cand.y,
        }),
      });
      const data = await res.json();
  ...
  const fillFromApiResponse = (data: any, _cand: GeoCandidate) => {
    ...
    if (useApr.length === 8) useApr = `${useApr.slice(0, 4)}-${useApr.slice(4, 6)}-${useApr.slice(6, 8)}`;

    // 용도지역: 기본개요 → 없으면 jijiguZone fallback
    ...
    if (!zone && data.jijiguZone) zone = String(data.jijiguZone).split(',')[0].trim();

    // 대지면적: API 값 → 없으면 landArea fallback
    let platAreaVal = title.platArea || '';
    if ((!platAreaVal || toNum(platAreaVal) === 0) && data.landArea) {
      platAreaVal = String(data.landArea);
    }
    ...
    const limits = zoneLimits[zone];
    setBuilding(prev => ({
      ...prev,
      bldNm:       title.bldNm        || '',
      platArea:    platAreaVal,
    ...
    if (limits) {
      setUserIn(prev => ({ ...prev, maxBcRat: String(limits.bc), maxVlRat: String(limits.vl) }));
    }
  };
  ...
  const handleZoneChange = (zone: string) => {
    ...
    const limits = zoneLimits[zone];
    if (limits) setUserIn(prev => ({ ...prev, maxBcRat: String(limits.bc), maxVlRat: String(limits.vl) }));
  };

  const handleCalc = () => setAnalyzed(true);
  ...
  const setUsr = (key: keyof UserInput) => (e: React.ChangeEvent<HTMLInputElement>) =>
    setUserIn(prev => ({ ...prev, [key]: commas(e.target.value) }));
  ...
  return (
    <div className="bld-page">
      {/* 탭 네비게이션 */}
      <div className="bld-tab-nav">
        <button
          className={`bld-tab${activeTab === 'analysis' ? ' active' : ''}`}
          onClick={() => setActiveTab('analysis')}
        >분석</button>
        <button
          className={`bld-tab${activeTab === 'settings' ? ' active' : ''}`}
      ...
      {/* 설정 탭 */}
      {activeTab === 'settings' && (
        <SettingsTab zoneLimits={zoneLimits} onSave={setZoneLimits} />
      )}
              ...
              <div className="bld-left">
                <div className="bld-group">
                  ...
                  <div className="bld-form">
                    ...
                    <div className="bld-field bld-field-yield">
                      <label>수익률</label>
                      <input readOnly value={yieldRate !== null ? `${yieldRate.toFixed(2)}%` : ''} placeholder="자동계산" />
                    </div>
                  </div>
                </div>
                ...
                <div className="bld-group">
                  ...
                  <div className="bld-form">
                    <div className="bld-field">
                      <label>건물명</label>
                      <input value={building.bldNm} onChange={setBld('bldNm')} placeholder="-" />
                      <span className="bld-unit" />
                    </div>
                    <div className="bld-field">
                      <label>대지면적</label>
                ...
                <button className="bld-btn-calc" onClick={handleCalc}>계산하기</button>
              </div>
              ...
              <div className="bld-right">
                {analyzed ? (
                  <>
                    <div className="bld-vis-row">
                      <div className="bld-svg-card">
                        <BuildingVisual
                          grndFlrCnt={grnd} ugrndFlrCnt={ugrnd}
                      ...
                      <div className="bld-info-card">
                        ...
                        <div className="bld-notice-inline">
                          {building.districtPlan && (
                            <strong>* 지구단위계획 해당 지역 ({building.districtPlan})<br /></strong>
                          )}
                          본 증축 분석은 조례 및 법령 기준으로만 산정한 결과입니다.
                          지구단위계획구역 내에서는 별도의 건폐율·용적률·높이 제한이
                          적용될 수 있으니 반드시 확인하시기 바랍니다.
                        </div>
                      </div>
                    </div>

                    <div className="bld-bottom-row">
                      ...
                      {floorItems.length > 0 && (
                        <div className="bld-floor-detail">
                          ...
                          <table className="bld-floor-table">
                            <thead>
                              <tr><th>층</th><th>구분</th><th>용도</th><th>면적(㎡)</th></tr>
                            </thead>
                            <tbody>
                              {floorItems
                                ...
                                .map((f, i) => (
                                  <tr key={i}>
                                    <td>{f.flrGbCd === '10' ? `B${f.flrNo}` : f.flrGbCd === '30' ? `옥탑${f.flrNo}` : `${f.flrNo}F`}</td>
                                    <td>{f.flrGbCd === '10' ? '지하' : f.flrGbCd === '30' ? '옥탑' : '지상'}</td>
                                    <td>{f.etcPurps || '-'}</td>
                                    <td style={{ textAlign: 'right' }}>{parseFloat(f.area).toFixed(1)}</td>
                                  </tr>
                                ))}
                            </tbody>
                          </table>
                  ...
                  <div className="bld-empty">
                    ...
                    <div style={{ marginTop: 8, fontSize: 12 }}>주소 입력 후 조회 버튼을 클릭하세요</div>
                  </div>
                )}
              </div>
            </div>
